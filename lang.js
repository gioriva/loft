...
var T = {
  ...
  de: "Ein Design-Loft im Herzen von Como. Architektonische Räume, einzigartige Atmosphäre."
  },
  "footer.nav": {
  it: "Navigazione",
  en: "Navigation",
  ...
  de: "Ein Design-Loft im Herzen von Como. Doppelte Raumhöhe, Wendeltreppe und jedes Detail für einen unvergesslichen Aufenthalt."
  },
  "home.hero.cta1": {
  it: "Prenota ora",
  en: "Book now",
  ...
  de: "Das private Badezimmer verfügt über Badewanne/Dusche, Haartrockner und hochwertige Handtücher."
  },
  "gal.cta": {
  ...
  "cookie.text": {
  it: "Questo sito usa cookie tecnici e, solo con il tuo consenso, cookie statistici (Google Analytics) per capire come viene visitato. Puoi accettare, rifiutare o cambiare idea quando vuoi.",
  en: "This site uses technical cookies and, only with your consent, analytics cookies (Google Analytics) to understand how it is visited. You can accept, refuse or change your mind at any time.",
  fr: "Ce site utilise des cookies techniques et, uniquement avec votre consentement, des cookies statistiques (Google Analytics). Vous pouvez accepter, refuser ou changer d'avis à tout moment.",
  de: "Diese Website verwendet technische Cookies und, nur mit Ihrer Zustimmung, Statistik-Cookies (Google Analytics). Sie können zustimmen, ablehnen oder Ihre Wahl jederzeit ändern."
  },
  "cookie.more": {
...
function applyLang(lang) {
  ...
  for (var i = 0; i < els.length; i++) {
    var el = els[i], key = el.getAttribute('data-i18n'), entry = T[key];
    if (!entry) continue;
...
function setLang(lang) { applyLang(lang); updateToggle(lang); }
function initLang() {
  var lang = readLang();
  applyLang(lang); updateToggle(lang);
  var btns = document.querySelectorAll('.lang-btn');
  for (var i = 0; i < btns.length; i++) {
    (function(btn){ btn.addEventListener('click', function(){ setLang(btn.getAttribute('data-lang')); }); })(btns[i]);
  }
}
